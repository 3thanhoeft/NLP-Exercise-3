...
esp_data.add_row(["PRON + VERB (4,95%)", "NOUN + ADJ (3,79%)", "NOUN + ADJ (3,85%)", "VERB + SCONJ (3,0%)"])
...
# Finally, in order to make table 2 from 7. Apendix, we need to add up percentages
# and create the corresponding prettytables.
def numbers_sum (percentage_x_genre):
    '''This function takes the first most frequent five pair of PoS combinations
    of each genre per language and returns the sum of those five percentages.'''
    numbers_sum = 0
    i = 0
    while i < 5:
        numbers_sum = numbers_sum + percentage_x_genre[i][0]
        i = i+1
    numbers_sum_round = round(numbers_sum,4)    
    return numbers_sum_round

# In the following lines of code, we are putting the different outputs of the
# function in different variables to use later on to make the prettytables.
esp_narrative_numbers_sum = numbers_sum(percentage_esp_narrative)
esp_poetry_numbers_sum = numbers_sum(percentage_esp_poetry)
esp_speech_numbers_sum = numbers_sum(percentage_esp_speech)
esp_song_numbers_sum = numbers_sum(percentage_esp_song)

eng_narrative_numbers_sum = numbers_sum(percentage_eng_narrative)
eng_poetry_numbers_sum = numbers_sum(percentage_eng_poetry)
eng_speech_numbers_sum = numbers_sum(percentage_eng_speech)
eng_song_numbers_sum = numbers_sum(percentage_eng_song)

gre_narrative_numbers_sum = numbers_sum(percentage_gre_narrative)
gre_poetry_numbers_sum = numbers_sum(percentage_gre_poetry)
gre_speech_numbers_sum = numbers_sum(percentage_gre_speech)
gre_song_numbers_sum = numbers_sum(percentage_gre_song)

# For each language, we add and round all the results to get the overall
# percentages for the top 5, regardless of genres:
esp_4_genres = round(esp_narrative_numbers_sum + esp_poetry_numbers_sum + esp_speech_numbers_sum + esp_song_numbers_sum, 2)
eng_4_genres = round(eng_narrative_numbers_sum + eng_poetry_numbers_sum + eng_speech_numbers_sum + eng_song_numbers_sum, 2)
gre_4_genres = round(gre_narrative_numbers_sum + gre_poetry_numbers_sum + gre_speech_numbers_sum + gre_song_numbers_sum, 2)

# We create the different prettytables. In some of them we have calculated the
# mean manually, without using the Python built-in function:
esp_numbers_sum = PrettyTable()
esp_numbers_sum.field_names = ["Narrative", "Speech", "Poetry", "Song"]
esp_numbers_sum.add_row([esp_narrative_numbers_sum, esp_speech_numbers_sum, esp_poetry_numbers_sum, esp_song_numbers_sum])
esp_numbers_sum.title = "SPANISH"

eng_numbers_sum = PrettyTable()
eng_numbers_sum.field_names = ["Narrative", "Speech", "Poetry", "Song"]
eng_numbers_sum.add_row([eng_narrative_numbers_sum, eng_speech_numbers_sum, eng_poetry_numbers_sum, eng_song_numbers_sum])
eng_numbers_sum.title = "ENGLISH"

gre_numbers_sum = PrettyTable()
gre_numbers_sum.field_names = ["Narrative", "Speech", "Poetry", "Song"]
gre_numbers_sum.add_row([gre_narrative_numbers_sum, gre_speech_numbers_sum, gre_poetry_numbers_sum, gre_song_numbers_sum])
gre_numbers_sum.title = "GREEK"

number_per_language = PrettyTable()
number_per_language.field_names = ["SPANISH", "ENGLISH", "GREEK"]
number_per_language.add_row([round((esp_4_genres/4),2), round((eng_4_genres/4),2), round((gre_4_genres/4),2)])
number_per_language.title = "ALL LANGUAGES"

sum_narrative = round(esp_narrative_numbers_sum + eng_narrative_numbers_sum + gre_narrative_numbers_sum, 2)
sum_speech = round(esp_speech_numbers_sum + eng_speech_numbers_sum + gre_speech_numbers_sum, 2)
sum_poetry = round(esp_poetry_numbers_sum + eng_poetry_numbers_sum + gre_poetry_numbers_sum, 2)
sum_song = round(esp_song_numbers_sum + eng_song_numbers_sum + gre_song_numbers_sum, 2)
    
number_per_genre = PrettyTable()
number_per_genre.field_names = ["Narrative", "Speech", "Poetry", "Song"]
number_per_genre.add_row([round((sum_narrative/3),2), round((sum_speech/3),2), round((sum_poetry/3),2), round((sum_song/3),2)])
number_per_genre.title = "ALL GENRES"

print("Adding up percentages to cross-compare heterogeneity/homogeneity:")
print("The larger the number in comparison to the others, the more homogeneous ")
print("that language or genre in comparison to the others- since the higher ")
print("that number is, the more that language/genre tends to use the same PoS combinations.")
print(esp_numbers_sum, '\n')
print(eng_numbers_sum, '\n')
print(gre_numbers_sum, '\n')
print(number_per_language, '\n')
